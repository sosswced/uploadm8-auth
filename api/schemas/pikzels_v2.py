...
class PikzelsV2PromptBody(PikzelsV2TelemetryFields):
    """Thumbnail prompt — ``persona`` and ``style`` are mutually exclusive (Pikzels API)."""
...
class PikzelsV2RecreateBody(PikzelsV2TelemetryFields):
    """Pikzels v2 create-from-image (Recreate™). ``persona`` and ``style`` are mutually exclusive."""
...
class PikzelsV2PikzonalityUpdateBody(PikzelsV2TelemetryFields):
