...
transcript, music ID + genre, background/YAMNet sound, emotional_tone from audio context when present, GPS/lat-lon + place names,
...
CATEGORY: {category}
USER ACCOUNT SETTINGS (UI): caption_style={caption_style} caption_tone={caption_tone}
...
def _specificity_bonus(caption: str, title: str, scene_graph: Dict[str, Any]) -> float:
    ...
    for part in re.split(r"\W+", ocr.lower()):
        if len(part) > 3:
            toks.append(part)
    for g in ("display", "start_display", "road", "city", "state"):
