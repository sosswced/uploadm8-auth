...
def _coerce_hashtag_list(v: Any) -> list[str]:
    """Normalize any hashtag input into a clean list of bare-token strings.

    Accepts:
      - Real lists/tuples of strings.
      - JSON-encoded strings like ``'["tester","qwe"]'`` (single OR double encoded).
      - Comma-separated text like ``"tester, qwe"``.

    Previously this function silently dropped any item starting with ``[`` or
    ``"`` to defend against the historic ``#"["tester" #"qwe"]"`` corruption.
    That defence also discarded *valid* JSON-string payloads on the way back in
    from the DB, so a re-save would wipe the user's hashtags.  Now we parse the
    JSON and recover the items instead.
    """
    if v is None:
        return []

    def _clean(token: Any) -> str:
        s = token if isinstance(token, str) else str(token or "")
        return re.sub(r"[^a-z0-9_]", "", s.lower().replace("#", ""))

    def _emit(items: list[Any], out: list[str]) -> None:
        for item in items:
            if item is None:
                continue
            ...
            if isinstance(item, str):
                ...
                if not s:
                    continue
                # Recover JSON payloads that were stuffed into a list cell.
                if s.startswith("[") or (s.startswith('"') and s.endswith('"')):
                    try:
                        ...
                        # Not actually JSON — strip the bare quote/bracket noise
                        # and continue with the cleaned token.
                        s = s.strip('"[]')
            else:
                s = str(item).strip()
            cleaned = _clean(s)
    ...
    if isinstance(v, str):
        ...
        if not s:
            return []
        # JSON string (possibly double-encoded) — peel and recurse.
        if s.startswith("[") or (s.startswith('"') and s.endswith('"')):
            try:
                parsed = json.loads(s)
                if isinstance(parsed, list):
...
async def save_user_content_preferences(conn, user: dict[str, Any], payload: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    email_notifications = _coerce_bool(p.get("email_notifications"), True)
    auth_security_alerts = _coerce_bool(p.get("auth_security_alerts"), True)
    digest_emails = _coerce_bool(p.get("digest_emails"), True)
    ...
    ai_service_music_detection = _coerce_bool(p.get("aiServiceMusicDetection", p.get("ai_service_music_detection")), True)
    ai_service_audio_summary = _coerce_bool(p.get("aiServiceAudioSummary", p.get("ai_service_audio_summary")), True)
    ai_service_emotion_signals = _coerce_bool(p.get("aiServiceEmotionSignals", p.get("ai_service_emotion_signals")), False)
    ai_service_caption_writer = _coerce_bool(p.get("aiServiceCaptionWriter", p.get("ai_service_caption_writer")), True)
    ai_service_thumbnail_designer = _coerce_bool(
    ...
    try:
        ...
        await conn.execute(
            ...
            hashtag_position,
            max_hashtags,
            # NOTE: pass Python list/dict directly. The asyncpg pool registers a
            # JSONB codec (see core.helpers._init_asyncpg_codecs); calling
            # json.dumps() here used to combine with that codec to *double*-
            # encode the value, leaving rows like `'"["tester","qwe"]"'` on disk
            # which then surfaced as the malformed `#"["tester" #"qwe"]"` tag.
            always,
            blocked,
            platform,
            email_notifications,
        ...
        if any(k in p or sk in p for k, sk in zip(caption_keys, caption_snake)) or any(
            ...
            try:
                ...
                if "thumbnailDefaultPersonaId" in p or "thumbnail_default_persona_id" in p:
                    ...
                    if pid_s:
                        try:
                            puuid = uuid.UUID(pid_s)
                        except (ValueError, AttributeError, TypeError):
                            pid_s = None
                        if pid_s:
                            row_cp = await conn.fetchrow(
                                "SELECT id::text FROM creator_personas WHERE id = $1::uuid AND user_id = $2::uuid",
                                puuid,
                                uid,
                            )
                            if not row_cp:
                ...
                # Pass dict directly — codec encodes once. See note in main UPDATE above.
                await conn.execute(
        ...
        try:
            await conn.execute(
                """UPDATE user_preferences SET
                    ...
                    always_hashtags=$10, blocked_hashtags=$11,
                    platform_hashtags=$12, email_notifications=$13,
                ...
                hashtag_position,
                max_hashtags,
                # Pass Python list/dict directly — codec encodes once.
                always,
                blocked,
                platform,
                email_notifications,
