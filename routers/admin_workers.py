...
async def admin_workers_memory_debug(
    ...
    from services.worker_memory_debug import build_worker_memory_debug_payload

    if core.state.db_pool is None:
        raise HTTPException(status_code=503, detail="Database unavailable")
    return await build_worker_memory_debug_payload(
        db_pool=core.state.db_pool,
        redis_client=core.state.redis_client,
        worker_id=worker_id,
        samples=samples,
    )
