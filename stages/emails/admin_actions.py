"""
UploadM8 — Phase 4b: Admin Action Emails
=========================================
...
Both fire TO THE USER being acted upon, so they know what happened
and feel the human touch behind the action.
"""
...
async def send_admin_wallet_topup_email(
    ...
    (or wherever the admin UI triggers credit_wallet).
    ...
    html = email_shell(
        ...
        body_rows=(
            ...
            + stat_grid(
                ("Token Type",  wt_short),
                ("Tokens Added", f"+{tokens_added:,}"),
                ("New Balance",  f"{new_balance:,} {wt_short}" if new_balance else "See dashboard"),
            )
            + tinted_box(
                f'<p style="margin:0;color:#d1d5db;font-size:14px;line-height:1.65;">'
