...
async def _session_user_readonly_no_wallet(
    request: Request,
    authorization: Optional[str] = Header(None),
):
    return await get_current_user_readonly_no_wallet(request, authorization)
...
async def get_catalog_content(
    ...
    user: dict = Depends(_session_user_readonly_no_wallet),
):
    """
