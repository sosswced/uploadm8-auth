...
Mirrors worker.py order after download:
  ...
  video intelligence (optional) -> dashcam OSD (HUD OCR; GPS backfill if no .map) -> thumbnail ->
  caption (title/caption/hashtags) -> merge_signal_hashtags_into_ctx
...
def log_stage_skip(logger: logging.Logger, stage_name: str, reason: str) -> None:
    logger.info("%s skipped: %s", stage_name, reason)
...
def _ctx_snapshot(ctx) -> Dict[str, Any]:
    ...
    return {
        ...
        "timeline_events_count": _timeline_events_count(ctx),
        "errors_count": len(getattr(ctx, "errors", []) or []),
    }


def _timeline_events_count(ctx) -> int:
    arts = getattr(ctx, "output_artifacts", None) or {}
    raw = arts.get("timeline_story") if isinstance(arts, dict) else None
    if not raw:
        return 0
    try:
...
def _persist_thumbnail_outputs(ctx, dest_root: Path) -> Dict[str, str]:
    from tools.thumbnail_output_helpers import persist_thumbnail_outputs

    return persist_thumbnail_outputs(ctx, dest_root)


def _print_thumbnail_outputs(saved: Dict[str, str], ctx) -> None:
    from tools.thumbnail_output_helpers import print_thumbnail_outputs
...
def _print_billing_block(
    ...
    print(
        f"  Flags for this estimate: billing_use_ai={use_ai}, "
...
async def run_pipeline(args: argparse.Namespace) -> int:
    ...
    from stages.dashcam_osd_stage import run_dashcam_osd_stage
    ...
    if getattr(args, "thumbnail_out_dir", ""):
        thumb_out_dir = Path(args.thumbnail_out_dir).resolve()
    elif dump_dir:
        thumb_out_dir = dump_dir / "thumbnails"
    else:
        thumb_out_dir = Path(__file__).resolve().parent / "_simulate_thumbnails"
    thumbnail_outputs: Dict[str, str] = {}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        want_telemetry = bool(us0.get("trill_enabled"))
        ...
        # -- Dashcam OSD (burned-in HUD; same slot as worker — may backfill telemetry + PADUS) --
        if not getattr(args, "skip_dashcam_osd", False):
            try:
                ctx = await run_dashcam_osd_stage(ctx)
            except SkipStage as e:
                log_stage_skip(log, "Dashcam OSD", e.reason)
            except Exception as e:
                log.warning("Dashcam OSD error: %s", e)
        else:
            log.info("Dashcam OSD skipped (--skip-dashcam-osd)")
        if dump_dir:
            _dump_ctx(ctx, dump_dir, "57_dashcam_osd")
        if stop_after == "dashcam_osd":
            log.info("Stopping after dashcam_osd (--stop-after-stage=dashcam_osd)")
            return 0

        # -- Scene-story + timeline (worker parity: after OSD, before hydration) --
        try:
            ...
            if not isinstance(ctx.output_artifacts, dict):
            ...
            log.info(
                "scene_story built (%d chars), timeline events=%d",
                len(_scene_story),
                len(_timeline_events),
            )
        except Exception as tl_e:
            log.warning("scene_story/timeline build skipped: %s", tl_e)
        if dump_dir:
            _dump_ctx(ctx, dump_dir, "59_timeline")

        # -- Canonical hydration payload (matches worker) --
        try:
            ctx = await run_hydration_payload_stage(ctx)
        except Exception as e:
            log.warning("hydration_payload_stage: %s", e)
        if dump_dir:
            _dump_ctx(ctx, dump_dir, "58_hydration_payload")
        if stop_after == "hydration_payload":
            log.info("Stopping after hydration_payload (--stop-after-stage=hydration_payload)")
            return 0
        ...
        try:
            ...
            if getattr(ctx, "thumbnail_path", None) and Path(ctx.thumbnail_path).exists():
                log.info("Thumbnail partial output available at %s — persisting copies", ctx.thumbnail_path)

        if ctx.thumbnail_path and Path(ctx.thumbnail_path).exists():
            shutil.copy2(ctx.thumbnail_path, out_thumb)
        try:
            thumbnail_outputs = _persist_thumbnail_outputs(ctx, thumb_out_dir)
            if thumbnail_outputs.get("winner"):
                log.info("Thumbnail outputs persisted under %s", thumb_out_dir)
                for label, path in sorted(thumbnail_outputs.items()):
                    log.info("  thumb %s -> %s", label, path)
            elif ctx.thumbnail_path:
                log.info("Wrote legacy copy %s", out_thumb)
        except Exception as thumb_copy_e:
            log.warning("Thumbnail output persist failed: %s", thumb_copy_e)
        if dump_dir:
        ...
        # -- Caption (title / caption / hashtags) + signal hashtag merge (same as worker) --
        if not getattr(args, "skip_caption", False):
            try:
                ...
                try:
                    ...
                    log.warning("signal_hashtags merge failed (non-fatal): %s", sig_e)
            except SkipStage as e:
    ...
    print(f"Thumbnail (legacy): {out_thumb if out_thumb.exists() else '-'}")
    _print_thumbnail_outputs(thumbnail_outputs, ctx)
    try:
...
def main() -> None:
    ...
    p.add_argument(
        ...
        help="Comma-separated platforms (Pikzels renders one styled thumb per platform)",
    )
    ...
    p.add_argument("--telemetry-map", default="", dest="telemetry_map",
                   help="Path to .map telemetry file for Trill/drive testing")
    ...
    p.add_argument(
        "--skip-dashcam-osd",
        action="store_true",
        dest="skip_dashcam_osd",
        help="Skip burned-in HUD OCR + telemetry backfill (matches worker when disabled)",
    )
    ...
    p.add_argument(
        "--thumbnail-out-dir",
        default="",
        dest="thumbnail_out_dir",
        help="Copy winner + per-platform styled JPEGs here (default: <debug-dump-dir>/thumbnails or tools/_simulate_thumbnails)",
    )
