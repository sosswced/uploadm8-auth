...
These cover the three highest-impact lifecycle moments that directly affect
churn and revenue recovery. Wire each one to its trigger point as documented
    ...
    html = email_shell(
        ...
        body_rows=(
            ...
            + tinted_box(
                ...
                f'letter-spacing:1px;">Payment Details</p>'
                f'<p style="margin:0;color:#ffffff;font-size:16px;font-weight:600;">'
                f'${amount:.2f} — {plan}</p>'
...
async def send_trial_ending_reminder_email(
    ...
    html = email_shell(
        ...
        body_rows=(
            ...
            + tinted_box(
                f'<p style="margin:0 0 8px;color:#ffffff;font-size:15px;font-weight:600;">'
                f'Want to cancel?</p>'
...
async def send_low_token_warning_email(
    ...
    during token spend. Check inside spend_tokens() in app.py after deducting:
        ...
        # After spend_tokens() call in the upload worker or API:
    ...
    balance_color = "#ef4444" if current_balance <= 2 else "#f97316"
    urgency = "critically low" if current_balance <= 2 else "running low"
    ...
    html = email_shell(
        ...
        body_rows=(
            intro_row(
                f"Your {wt_short} balance is {urgency} 🪙",
            ...
            + tinted_box(
                ...
                f'<td style="text-align:center;padding:0 24px;">'
                f'<p style="margin:0;color:#6b7280;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Current Balance</p>'
                f'<p style="margin:6px 0 0;color:{balance_color};font-size:32px;font-weight:700;">{current_balance}</p>'
                f'<p style="margin:4px 0 0;color:#6b7280;font-size:13px;">{wt_short} tokens remaining</p>'
                f'</td></tr></table>',
                hex_color="#7c3aed",
            )
