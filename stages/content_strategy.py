...
from .publish_stage import _youtube_avoid_shorts_markers_for_rights
...
def build_content_strategy(
    ...
    *,
    ...
    # YouTube: catalogue audio in a clip ≥60s cannot be positioned as a Short (Content ID).
    # Steer M8 away from #shorts so publish_stage's sanitizer is not fighting the model alone.
    if _youtube_avoid_shorts_markers_for_rights(ctx) and "youtube" in platform_targets:
        yt = platform_targets["youtube"]
        cons = dict(yt.get("constraints") or {})
        bp = [str(x).strip() for x in (cons.get("banned_phrases") or []) if str(x).strip()]
        banned_shorts_tokens = (
            "#shorts",
            "#YouTubeShorts",
            "#youtubeshorts",
            "#YTShorts",
        ...
        seen = {b.lower() for b in bp}
        for tok in banned_shorts_tokens:
            if tok.lower() not in seen:
                bp.append(tok)
                seen.add(tok.lower())
        cons["banned_phrases"] = bp[:24]
        yt["constraints"] = cons
